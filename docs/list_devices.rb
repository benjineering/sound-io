...
def print_device(device, is_default) {
  ...
  unless device.probe_error.nil? do
  ...
  unless device.current_layout.channels.empty? do
    puts ' current layout'
...
def list_devices(sio) {
  ...
  sio.input_devices.each_with_index do |device, i|
    print_device(device, default_input_device_index == i)
  end

  puts '--------Output Devices--------'
  sio.output_devices.each_with_index do |device, i|
    print_device(device, default_output_device_index == i)
  end
end
